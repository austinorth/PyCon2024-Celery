...
def deposit_and_send_sms_using_group():
    ...
    sig_whatsapp = task_send_whatsapp.si(
        1234, "Your account has been credited with $5000"
    )
    sig_whatsapp_1 = task_send_whatsapp.si(
        4321, "Your account has been credited with $3000"
    )
    sig_whatsapp_2 = task_send_whatsapp.si(
        5678, "Your account has been credited with $7000"
    )

    # Create the group of a signatures
    grp = group(
        sig_sms, sig_sms_1, sig_sms_2, sig_whatsapp, sig_whatsapp_1, sig_whatsapp_2
    )
